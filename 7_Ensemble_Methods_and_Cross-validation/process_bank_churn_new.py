...
def scale_numeric_features(
    ...
    Binary 0/1 columns are not scaled.

    Args:
        train_inputs: Training features DataFrame.
        val_inputs: Validation features DataFrame.
        numeric_cols: List of numeric column names.
        scaler_numeric: If True, apply StandardScaler to non-binary numeric columns.
    ...
    def _is_binary_01(series: pd.Series) -> bool:
        # Drop NaNs, handle bool and numeric 0/1.
        uniques = set(series.dropna().unique().tolist())
        return len(uniques) > 0 and uniques.issubset({0, 1, True, False})

    binary_cols = [c for c in numeric_cols if _is_binary_01(train_processed[c])]
    scale_cols = [c for c in numeric_cols if c not in set(binary_cols)]

    # Nothing to scale (only binary numeric columns)
    if not scale_cols:
        return train_processed, val_processed, None

    scaler = StandardScaler().fit(train_processed[scale_cols])
    train_processed.loc[:, scale_cols] = scaler.transform(train_processed[scale_cols])
    val_processed.loc[:, scale_cols] = scaler.transform(val_processed[scale_cols])
